...
$cover_row = spoPrintRow(spoPrintQuery($conn, "SELECT * FROM cover_page WHERE id_school IS NULL AND ac IS TRUE"));
...
if(!empty($schoolId)){
  ...
  $cover_row = spoPrintRow(spoPrintQuery($conn, "SELECT * FROM cover_page WHERE id_school = $id_school_get AND ac IS TRUE"));
...
<p class='com'><strong> комиссий ИПМЭиТ на ".$cover_year." учебный 
<p class='com'><strong>год</strong><p class='com_break'>

<p>В целях организации учебного процесса и проведения процедуры зачета результатов обучения по отдельным дисциплинам (модулям) и (или) отдельным практикам, освоенным (пройденным) обучающимся при получении среднего профессионального образования и (или) высшего образования, а также дополнительного образования (при наличии), для лиц, обучающихся в ИПМЭиТ, восстанавливаемых на обучение, или зачисляемых по переводу, в соответствии с Порядком зачета результатов освоения обучающимися курсов, дисциплин (модулей), практики, дополнительных образовательных программ, утвержденных приказом Министерства науки и высшего образования РФ от 17.08.2021 № 1669.
<p><strong>ОБЯЗЫВАЮ:</strong>
<p>Создать аттестационные комиссии по переаттестации ранее изученных дисциплин по Институту промышленного менеджмента экономики и торговли в составе в соответствии с Приложением 1." : "<p class='com_break'><p>Для случаев проведения процедуры зачета результатов обучения, а также в случае перевода, восстановления, возврата из академического отпуска для лиц, обучающихся в  ".$dp.", просим утвердить на ".$cover_year." учебный год комиссии по переаттестации ранее изученных дисциплин в соответствии с Приложением.";
...
$num_app=1;
  ...
  $html.= empty($id_school_get) ? '<header><p class="noindent "align="right">Приложение'.$app.'<p class="school">Составы аттестационных комиссий по Институту промышленного менеджмента, экономики и торговли на '.$cover_year.' учебный год</p>'
   : '<p class="school">Составы аттестационных комиссий по переаттестации ранее изученных дисциплин по '.$dp.'<br> на '.$cover_year.' учебный год</p>';
 
  $cert_query = empty($id_school_get) ? "SELECT * FROM ac_info ORDER BY id_com_spo" : "SELECT * FROM ac_info WHERE id_school = $id_school_get ORDER BY id_com_spo";
...
while ($cert_row = spoPrintRow($cert_result)) {
  ...
  foreach ($teachers as &$tch) {
    ...
    $tch = !empty($tch_name) ? '<tr><td>'.htmlspecialchars($tch_name,ENT_QUOTES|ENT_SUBSTITUTE,'UTF-8').'</td>
...
$dir_s = remove_dubs($dir_s);
  ...
  $html.=  "<strong>АК № ".$id_com.":</strong>";
    if ($id_cert == 60) {
    $html.= '<br><em>По направлениям подготовки бакалавров, магистров и специалистов, реализуемым 
    <br>в Институте промышленного менеджмента, экономики и торговли, контингент 
    <br>иностранных граждан из стран с визовым режимом очной формы обучения.</em><br>';
  }

  if ($id_cert == 61) {
    $html.= "<br><em>По направлениям подготовки бакалавров, магистров и специалистов, реализуемым 
    <br>в Институте промышленного менеджмента, экономики и торговли, для 
    <br>рассмотрения особых случаев.</em><br>";
