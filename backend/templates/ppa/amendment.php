...
$cover_row = spoPrintRow(ppaPrintQuery($conn, "SELECT * FROM cover_page WHERE id_school IS NULL AND spo = false AND add_edit IS TRUE"));
$max_app = spoPrintRow(ppaPrintQuery($conn, "SELECT COUNT (DISTINCT id_school) FROM certification_info WHERE add_edit IS TRUE"))['count'];
$app_txt = $max_app > 1 ? "Приложениями 1 - ".$max_app : "Приложением";

if($schoolId){
  $id_school_get = $schoolId;
  $id_result = ppaPrintQuery($conn, "SELECT * FROM school WHERE id_school = $id_school_get");
  $cover_row = spoPrintRow(ppaPrintQuery($conn, "SELECT * FROM cover_page WHERE id_school = $id_school_get AND spo = false AND add_edit IS TRUE"));
...
$caps = $cover_row['caps'];
$doc_type = empty($id_school_get) ? 'РАСПОРЯЖЕНИЕ' : 'СЛУЖЕБНАЯ ЗАПИСКА';
$opt = $cover_row['opt'];
$dir = $cover_row['dir'];
...
$html.= $doc_type.'<p>'; 
  ...
  $html.= "<p class='com_break school_1'><u>".$cover_date_add."</u>"." "."<strong>№</strong> <u>".$num_add."</u>";
...
$html.= empty($id_school_get) ? "<p class='com_break'><p class='com'><strong>О внесении дополнений в 
<p class='com'><strong>Распоряжение от ".$cover_date."
  № ".$num."
<p class='com'><strong>«О составах комиссий для проведения
<p class='com'><strong>повторных аттестаций студентов 
<p class='com'><strong>ИПМЭиТ на ".$cover_year." учебный год»</strong><p class='com_break'>

<p >На основании ".$dir."
<p ><strong>ОБЯЗЫВАЮ:</strong>" : "";

$html.= empty($id_school_get) ? "<p>Внести ".$opt." в Распоряжение от ".$cover_date." № ".$num." «О составах комиссий для проведения повторных аттестаций студентов ИПМЭиТ на  
".$cover_year." учебный год» дополнив перечень комиссий в соответствии с ".$app_txt."." : "<p class='com_break'><p>В связи с производственной необходимостью для проведения второй повторной промежуточной аттестации по основным образовательным программам бакалавриата, специалитета, магистратуры на ".$cover_year."учебный год просим внести ".$opt." в составы комиссий для приема академических задолженностей студентов в соответствии с Приложением.";
...
$rp = $row['rp'];
  ...
  $cert_query = "SELECT * FROM certification_info WHERE id_school = $id_school AND add_edit IS TRUE ORDER BY id_com";
...
//if($id_school < $max_id && empty($id_school_get)) $html.='<div class="page-break"></div>';
  ...
  $html.= empty($id_school_get) ? $num_app.' к Распоряжению от '.$cover_date_add.' № '.$num_add.'</p></header>' : '</p></header>';
