...
try {
  ...
  assert.equal((await request.get('/api/v1/spo/context')).status(), 401);
  ...
  assert.equal((await request.post('/api/v1/spo/save', { data: {} })).status(), 403);
  const response = await request.get('/api/v1/spo/context');
  assert.equal(response.status(), 200);
  const data = (await response.json()).data;
  assert.ok(data.records.length > 0);
  assert.ok(data.records.every(r => r.kind === 'spo' && r.version));
  const first = data.records[0];
  for (const [name, args] of Object.entries({ order: {}, memo: { school: first.school }, table: { tableOnly: true, ids: [first.id] } })) {
    const result = await request.post('/api/v1/spo/print', { headers: { 'X-CSRF-Token': csrf }, data: { ...args, academicYear: '2026/2027' } });
    assert.equal(result.status(), 200, `Print ${name}`);
    const pdf = Buffer.from((await result.json()).data.pdf, 'base64');
    assert.equal(pdf.subarray(0, 5).toString(), '%PDF-');
    await writeFile(`backend/var/print-tests/spo-${name}.pdf`, pdf);
  }
  const page = await context.newPage();
  const errors = [];
  page.on('pageerror', error => errors.push(error.message));
  await page.goto('/attestation/spo');
  await page.getByRole('heading', { name: 'Комиссии СПО', exact: true }).waitFor();
  ...
  await page.goto('/attestation/spo/documents');
  ...
  await page.goto('/attestation/spo/commissions');
  await page.locator('.commissions-table').waitFor();
  await page.screenshot({ path: 'backend/var/print-tests/spo-ui.png', fullPage: true });
  ...
  await page.screenshot({path:'backend/var/print-tests/spo-mobile.png',fullPage:true});
  assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth+2),'Mobile page fits viewport');
  assert.deepEqual(errors, []);
  console.log('PASS: auth, CSRF, real SPO data, all 3 PDF modes, editor, cover form, PDF preview. No business data changed.');
} catch (error) {
  console.error('SPO browser check failed:', error.message.split('\n')[0]);
  process.exitCode = 1;
