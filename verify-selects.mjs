...
const school=dialog.getByRole('combobox').filter({hasText:'Выберите высшую школу'});await school.click();await expect(page.getByRole('listbox')).toBeVisible();await page.screenshot({path:'artifacts/select-modal.png',fullPage:true});await page.getByRole('option',{name:'Высшая школа производственного менеджмента',exact:true}).click();await expect(school).toContainText('Высшая школа производственного менеджмента');
...
await page.setViewportSize({width:390,height:844});await year.click();await expect(page.getByRole('listbox')).toBeVisible();const box=await page.getByRole('listbox').boundingBox();if(box.x<0||box.x+box.width>391)throw Error('Dropdown overflow');await page.screenshot({path:'artifacts/select-mobile.png',fullPage:true});
