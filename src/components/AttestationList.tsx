...
export function AttestationList({ kind, rows, onOpen, onRemove }: { kind: AttestationKind; rows: AttestationRecord[]; onOpen: (r: AttestationRecord) => void; onRemove: (r: AttestationRecord) => void }) {
  ...
  const name = (r: AttestationRecord) => (r.kind === 'spo' ? 'СПО' : r.kind === 'ac' ? 'АК' : 'ППА') + ' № ' + r.number + (r.kind === 'ppa' ? ' · ' + (c.schools.find(s => s.id_school === r.school)?.short || 'Школа не указана') : '');
  ...
  useEffect(()=>{
    if(shown.length>=filtered.length || !more.current)return;
    const observer=new IntersectionObserver(entries=>{if(entries[0]?.isIntersecting)setPage(p=>p+1);},{rootMargin:'150px'});
    observer.observe(more.current);return()=>observer.disconnect();
  },[kind,shown.length,filtered.length]);
  ...
  return <><div className="list-panel">
    ...
    <footer ref={more} className="table-footer"><span>{filtered.length} комиссий</span></footer>
  ...
  {preview && <SpoPdfPreview kind={kind} amendment={rows[0]?.amendment ?? false} tableOnly ids={printRows.map(r=>r.id)} onClose={()=>setPreview(false)}/>}
  </>;
}
