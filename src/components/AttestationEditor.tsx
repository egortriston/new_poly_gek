...
function Selection({ title, options, values, onApply, onClose }: { title: string; options: Option[]; values: string[]; onApply: (values: string[]) => void; onClose: () => void }) {
  ...
  return <Modal title={title} onClose={onClose} footer={<><span className="muted">Выбрано: {selected.length}</span><button className="button secondary" onClick={onClose}>Отмена</button><button className="button primary" onClick={() => onApply(selected)}>Подтвердить <Check size={16}/></button></>}><div className="search-field"><input aria-label="Поиск в справочнике" placeholder="Поиск по справочнику" value={query} onChange={e => setQuery(e.target.value)}/></div><div className="picker-list">{options.filter(o => o.label.toLocaleLowerCase('ru').includes(query.toLocaleLowerCase('ru'))).map(o => <label className={'picker-option ' + (selected.includes(o.id) ? 'selected' : '')} key={o.id}><input type="checkbox" checked={selected.includes(o.id)} onChange={e => setSelected(e.target.checked ? [...selected, o.id] : selected.filter(id => id !== o.id))}/><span><strong>{o.label}</strong><small>{o.description}</small></span></label>)}</div></Modal>;
}

export function AttestationEditor({ record, onSave, onClose }: { record: AttestationRecord; onSave: (record: AttestationRecord) => boolean; onClose: () => void }) {
  ...
  function save() { if (!/^\d+$/.test(draft.number.trim())) { setError('Укажите номер комиссии цифрами'); return; } if (onSave({ ...draft, number: draft.number.trim() })) { baseline.current = JSON.stringify(draft); setDraft({ ...draft }); setError(''); } else setError('Не удалось сохранить запись. Проверьте уникальность номера и доступность хранилища браузера.'); }
  const options: Record<GroupKey, Option[]> = {
    members: c.teachers.filter(t => (t.id_school === draft.school || draft.members.includes(t.id_teacher)) && t.id_teacher !== draft.chairman).map(t => ({ id: t.id_teacher, label: t.name_teacher, description: t.position_teacher })),
  ...
  return <div className="page-enter detail-page"><Breadcrumb label="Список комиссий" current={'Комиссия № ' + draft.number} onClick={() => dirty ? setLeaving(true) : onClose()}/><div className="page-heading detail-heading"><div><PageTitle>{ppa ? 'ППА' : draft.kind === 'spo' ? 'СПО' : 'АК'} № {draft.number}</PageTitle><p>{school?.name_school ?? 'Общеинститутская комиссия'}{draft.amendment ? ' · Изменения и дополнения' : ''}</p></div><div className="button-row"><button className="button secondary" onClick={() => setMetadata(true)}><Pencil size={15}/> Сведения</button><button className="button primary" onClick={save}><Check size={17}/> Сохранить</button></div></div>
    ...
    {role && <Modal title={role === 'chairman' ? 'Председатель' : 'Секретарь'} onClose={() => setRole(null)} footer={<button className="button primary" onClick={() => setRole(null)}>Подтвердить <Check size={16}/></button>}><Field label="Преподаватель"><AppSelect aria-label="Преподаватель" value={draft[role]} onChange={e => update(role, e.target.value)}><option value="">Не назначен</option>{c.teachers.filter(t => role === 'chairman' || t.id_teacher !== draft.chairman).map(t => <option key={t.id_teacher} value={t.id_teacher}>{t.name_teacher}</option>)}</AppSelect></Field></Modal>}
