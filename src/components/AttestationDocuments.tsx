...
export function AttestationDocuments({ kind, school, rows }: { kind: AttestationKind; school: string; rows: AttestationRecord[] }) {
  ...
  return <>
    ...
    <p className="info-note">Кнопки печати открывают макет документа для проверки, затем «Печать / PDF» вызывает диалог печати. В демо доступна проверка данных и пробная печать. Официальные документы будут формироваться по существующим унифицированным шаблонам после подключения сервера.</p>
