...
export function AppSelect({children,value,onChange,required,disabled,name,id,className,'aria-label':label,...rest}:SelectHTMLAttributes<HTMLSelectElement>) {
 ...
 return <Select.Root onOpenChange={() => setQuery('')} value={selected|| (required?'':EMPTY)} required={required} disabled={disabled} name={name} onValueChange={v=>{const next=v===EMPTY?'':v;onChange?.({target:{value:next},currentTarget:{value:next}} as ChangeEvent<HTMLSelectElement>);}}><Select.Trigger ref={triggerRef} id={id} aria-label={label} aria-describedby={rest['aria-describedby']} className={`app-select ${className??''}`}><span className="app-select-label">{options.find(o=>o.value===selected)?.label??empty?.label??'Выберите значение'}</span><Select.Icon className="app-select-arrow"><ChevronDown size={15}/></Select.Icon></Select.Trigger><Select.Portal container={container??undefined}><Select.Content className="app-select-menu" position="popper" sideOffset={6} collisionPadding={12}><div className="app-select-search"><input autoFocus aria-label="Поиск вариантов" placeholder="Введите для поиска…" value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(!['Escape','ArrowDown','ArrowUp','Tab'].includes(e.key))e.stopPropagation();}}/></div><Select.ScrollUpButton className="app-select-scroll"><ChevronUp size={14}/></Select.ScrollUpButton><Select.Viewport className="app-select-viewport">{!options.some(o=>matchesSearch(o.label, query))&&<div className="app-select-empty">Варианты не найдены</div>}{options.filter(o=>matchesSearch(o.label, query)).map(o=><Select.Item key={o.value} value={o.value||EMPTY} disabled={o.disabled||(required&&!o.value)} className="app-select-option"><Select.ItemText>{o.label}</Select.ItemText><Select.ItemIndicator className="app-select-check"><Check size={15}/></Select.ItemIndicator></Select.Item>)}</Select.Viewport><Select.ScrollDownButton className="app-select-scroll"><ChevronDown size={14}/></Select.ScrollDownButton></Select.Content></Select.Portal></Select.Root>;
}
