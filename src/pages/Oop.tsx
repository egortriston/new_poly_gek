...
export function Oop() {
  ...
  return <div className="page-enter hub-page oop-page">
    ...
    {!current ? <div className="oop-module-grid">{oopSections.map(s => {
      ...
      {current.id === 'formation' && program ? <OopFormation key={program.id_mep} programId={program.id_mep}/> : current.id === 'print' && program ? <OopPrint key={program.id_mep} programId={program.id_mep}/> : current.id === 'status' ? <OopStatus/> : current.id === 'data' ? <div className="people-hub-grid">{['Исходные данные для формирования сведений по направлениям подготовки', 'Общие исходные данные всех образовательных программ', 'Исходные данные для формирования сведений по образовательным программам'].map((title, index) => index === 2 ? <Link className="module-card available" key={title} to="/oop/data/programs"><span className="module-icon"><Database size={24}/></span><h2>{title}</h2><p>Матрица ПК ООП и формы обучения образовательных программ.</p><footer>Открыть раздел<ArrowUpRight size={17}/></footer></Link> : <section className="module-card" key={title}><span className="module-icon"><Database size={24}/></span><h2>{title}</h2><p>Таблицы этого блока будут перенесены следующим этапом.</p></section>)}</div> : <section className="scope-note"><current.icon size={28}/><h2>{current.program ? program ? program.id_program + ' · ' + program.name_program : 'Выберите образовательную программу' : 'Таблица статусов ООП'}</h2><p>Структура раздела перенесена. {current.id === 'print' ? 'Подключение существующей печатной формы — следующий этап.' : current.id === 'formation' ? 'Перенос формы заполнения — следующий этап.' : 'Перенос таблицы статусов — следующий этап.'}</p></section>}
    </>}
  </div>;
}
