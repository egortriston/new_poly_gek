...
import { readSession, signIn, signOut, type SessionUser } from './api';
...
type Store = { commissions: Commission[]; save: (c: Commission) => boolean; remove: (id: string) => boolean; authenticated: boolean; user: SessionUser | null; authLoading: boolean; authError: string; refreshSession: () => Promise<void>; login: (username: string, password: string) => Promise<void>; logout: () => Promise<void>; toast: string; notify: (message: string) => void; year: string; setYear: (year: string) => void };
...
export function StoreProvider({ children }: { children: ReactNode }) {
 ...
 const refreshSession = useCallback(async () => {
   ...
   try { setUser((await readSession()).user); }
 ...
 const login = async (username: string, password: string) => { const session = await signIn(username, password); setUser(session.user); setAuthError(''); };
 const logout = async () => { await signOut(); setUser(null); };
 return <Context.Provider value={{ commissions, save, remove, authenticated, user, authLoading, authError, refreshSession, login, logout, toast, notify, year, setYear }}>{children}</Context.Provider>;
}
